...
begin
    ...
    begin
        if PRESETn = '0' then
            ...
            request_valid <= '0'; request_cmd <= (others => '0');
                ...
                if rx_byte = FLAG then
                    if frame_active = '1' and (frame_count /= 0 or escape_pending = '1') then
                        if escape_pending = '1' or frame_count < 10 then
                            ...
                            if frame_buf(0) = VERSION and frame_buf(1) = REQUEST_TYPE and
                               dest = ("0000000" & my_address) then
    ...
    begin
        if PRESETn = '0' then
            ...
            case engine is
                ...
                when E_TRANSMIT =>
                    if tx_done = '1' then reply_count <= reply_count + 1; engine <= E_IDLE; end if;
            end case;
        end if;
    end process;
    ...
    begin
        if PRESETn = '0' then
            ...
            delay_ticks <= 0; delay_left <= 0; setup_ticks <= 0; wire_index <= 0; tx_escape <= '0';
        elsif rising_edge(PCLK) then
            tx_done <= '0';
