...
// signals exchanged with ROC_FIFO_CNTRL on SERDESCLK time domain 
   ...
   input    ew_fifo_we,    // WR data from ROCFIFO into current EWFIFO
	input    ew_ovfl,       // packet size has reached maximum allowed of 1023
   input [`DIGI_BITS-1:0]         ew_data,    // EW data from ROCFIFO controller
	input	[`EVENT_SIZE_BITS-1:0]   ew_size,    // beats max size is [9:0] from ROCFIFO controller in unit of 64-bit AXI_beats
	input	[`SPILL_TAG_BITS-1:0]	   ew_tag,     // EW tag counter withing SPILL	
...
// signals exchanged with EW_SIZE_STORE_AND_FETCH_CNTRL on DREQCLK time domain
	...
	input	   [`DDR_ADDRESS_BITS-1:0]	tag_cnt,    // EVTAG count: set the DDR address
...
// signals exchanged with TOP_SERDES on DREQCLK	
   ...
   input    last_word,                                // DATAREQ_LAST_WORD 
	output   [`AXI_BITS-1:0]             et_fifo_rdata,	// DATAREQ_DATA (fills 1/2 DTC packet)
...
//wire	ew_fifo0_full, ew_fifo1_full,  ew_fifo_full; // EW_FIFO almost full (to give time to stop DIGIFIFO read -> unused)
...
reg	[`DDR_ADDRESS_BITS-1:0] 	et_fifo0_cnt,  et_fifo1_cnt;
reg	[`DDR_ADDRESS_BITS-1:0] 	et_cnt;
...
reg	ew_fifo_starved;
reg	[1:0] hdr_cnt; // needs to count only to 2
...
reg	[`EVENT_TAG_BITS-1:0] evt_offset;
...
wire  ew_size_empty;
wire  [11:0]  ew_size_sync; // containts ew_size[9:0] + ew_ovfl[10] 
wire  ew_tag_empty;
...
wire  [11:0]   tag_size_sync; // tag_size[9:0] + tag_ovfl[10] 
wire	[`DDR_ADDRESS_BITS-1:0]   tag_cnt_sync;
wire 	[`EVENT_TAG_BITS-1:0]    tag_evt_sync;

reg   tag_empty_all, tag_empty_reg, tag_empty_latch;
reg   tag_empty_ren, tag_empty_pulse;
...
// after enabling multiple blocks per AXI transations
...
always@(posedge serdesclk, negedge resetn_serdesclk)
begin
   if(resetn_serdesclk == 1'b0)  ew_fifo_emptied	<= 1'b0;
	else
	begin
		ew_fifo_emptied	<= 0;
...
begin
   ...
   begin
      ...
      if (ddr_write_done_latch && !ddr_write_done_reg) 
         curr_ewfifo_rd 	<= ~curr_ewfifo_rd;
...
begin
   ...
   begin
      ...
      // use pulse indicating that TAG bus signals have been properly latched
      if (tag_empty_pulse) 
      begin
...
begin
   ...
   begin
      ...
      // Ensure that DDR read:
		...
		if ((wait_for_read || first_axi_read) && DDR_to_read == 0 && et_fifo0_fromDDR == 1)
		begin
			DDR_to_read		<=	1;
			first_axi_read	<= 0;
		...
		else if (wait_for_read & DDR_to_read == 1 && et_fifo1_fromDDR == 1)
		begin
			DDR_to_read		<=	0;
			start_read		<= 1;
			et_fifo1_clr	<=	1;
	...
	// set inital address based on ET_CNT, which is a local counter based on ET_SIZE within the spill
	// now that START_READ
	SET:
	begin
		araddr_o		<=	BURST_OFFSET*et_cnt;
		raddr_state	<=	CHECK;
	end
...
begin
    if(resetn_sysclk == 1'b0) begin
        rready_o       <=	1'b0;
        rdburst_cnt		<=	8'b0;
        wait_for_read	<=	1'b0;		// level started by end or RBURST and ended by START_READ
        ...
        begin
            ...
            first_rd_hdr	<= 0;
            second_rd_hdr	<= 0;
            // by commenting them, the first error or each will drive the ERROR output forever
            //header1_error	<=	0;
            //header2_error	<=	0;
            //data_error		<= 0;
            //event_error		<= 0;
        ...
        begin
            ...
            if (save_et_ovfl)	ovfl_detected	<=	save_et_ovfl;
        ...
        begin
            if(rdburst_cnt == et_blk)   begin
                ...
                wait_for_read	<=	1'b1;	// level from end of read to start of next one
                rready_o			<=	1'b0;
                rdata_state		<=	IDLE;
            end
            else begin
                if(rvalid_i) begin
                    ...
                    // Use EVT_OFFSET to make it become the Event Window Tag at the end of previous run 
                    // Check SIZE with HEADER1_ERROR and EVTAG info with EVENT_ERROR flag.
                    // Save seen and expected ONLY for first error.
                    if (hdr_cnt == 1)   begin
                        //if ( first_rd_hdr[`EVENT_TAG_BITS-1:0] != evt_offset) begin
                            //event_error <= 1;
                            //if (event_error == 1'b0) begin
                                //evt_expc    <= {16'b0, evt_offset};
                                //evt_seen    <= {16'b0, first_rd_hdr[`EVENT_TAG_BITS-1:0]};
                            //end
                        //end
                        if ( event_error == 0 && first_rd_hdr[`EVENT_TAG_BITS-1:0] != evt_offset) begin
                            event_error <= 1;
                            evt_expc    <= {16'b0, evt_offset};
                            evt_seen    <= {16'b0, first_rd_hdr[`EVENT_TAG_BITS-1:0]};
                        end
                            
                        //if  (   first_rd_hdr[63] != et_ovfl ||	
                                //first_rd_hdr[48+`EVENT_SIZE_BITS-1 : 48] != et_size) begin
                            //header1_error  <= 1;
                            //if (header1_error == 1'b0) begin
                    ...
                    if (hdr_cnt == 0)	begin
                        ...
                        if (ovfl_detected==1 && hdr_cnt==2) begin
                            ovfl_detected	<= 0;
...
begin
   ...
   begin
      ...
      if (wait_cnt_dreq  > WAIT_FOR_RST) 
      begin
         et_pckt_empty_pulse <= et_pckt_empty_latch && ~et_pckt_empty_sync;
      end
   end
end
...
begin
   ...
   begin
      ...
      ew_empty_latch <= ew_empty_reg;
      
      ew_empty_ren   <= ew_empty_all && ~ew_empty_reg;
      ew_empty_pulse <= ew_empty_reg && ~ew_empty_latch;
   end  
end 
...
//
// logic to clean spurious TAG_EMPTY at reset
// and latch TAG_EMPTY rising edge
always@(posedge sysclk, negedge resetn_sysclk)
begin
   if(resetn_sysclk == 1'b0)
   begin
   ...
   begin
      if (wait_cnt  > WAIT_FOR_RST) 
         tag_empty_all     <= (!(tag_size_empty || tag_cnt_empty || tag_evt_empty)) ? 1'b1: 1'b0;
         
      tag_empty_reg  <= tag_empty_all;
      tag_empty_latch<= tag_empty_reg;
      tag_empty_ren  <= tag_empty_all && ~tag_empty_reg;
      tag_empty_pulse<= tag_empty_reg && ~tag_empty_latch;
   end
end   
