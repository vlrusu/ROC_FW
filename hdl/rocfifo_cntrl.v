...
//global signals
    ...
    input	[`ROCFIFO_DEPTH-1:0] rocfifo_wrcnt,  // WRCNT from ROCFIFO0 - unused 

    input axi_start_on_serdesclk,   // ROCFIFO data is being written to DDR. Can start processing next HB.

	output reg  rocfifo_re,	        // ROCFIFO0 read enable
...
begin
    ...
    begin
        ...
        begin
            ...
            if (rocfifo_empty == 1'b0)
            begin
                // set which EW_FIFO has to receive the data
                curr_ewfifo_wr <= ~curr_ewfifo_wr;
                
                // diagnostics: all ROCFIFOs should report data header from same TAG
                ew_tag   <= rocfifo_data[19:0];
                
                curr_size <= rocfifo_data[31:20];              // size from current ROCFIFO in units of 32-bit words
                full_size <= full_size + rocfifo_data[31:21];  // total no. of packets (64-bit) for current event window
                ew_size  <= ew_size + rocfifo_data[31:21];     // size summed over all ROCFIFOs in units of packets, or 64-bit words
